...
class Admin {
    ...
    public function enqueue_block_editor_assets() {
        wp_enqueue_script(
            ...
            filemtime(plugin_dir_path(dirname(__FILE__)) . 'build/editor.js')
        );
    }
    ...
    public function enqueue_management_scripts($hook) {
        if ($hook === 'edit.php' && isset($_GET['post_status']) && $_GET['post_status'] === 'queued') {
            wp_enqueue_script(
                'wp-queue-settings-panel-script',
                plugins_url('/build/settings-panel.js', dirname(__FILE__)),
                array('wp-element', 'wp-components', 'wp-data', 'wp-api', 'wp-api-fetch', 'wp-redux-routine'),
                filemtime(plugin_dir_path(dirname(__FILE__)) . 'build/settings-panel.js'),
                true
            );

            wp_localize_script('wp-queue-settings-panel-script', 'wpQueuePluginData', array(
            ...
            wp_enqueue_script(
                'drag-drop-reorder',
                plugins_url('/build/drag-drop-reorder.js', dirname(__FILE__)),
    ...
    public function enqueue_management_styles($hook) {
        if ($hook === 'edit.php' && isset($_GET['post_status']) && $_GET['post_status'] === 'queued') {
            wp_enqueue_style(
                ...
                plugins_url('/build/settings-panel.css', dirname(__FILE__)),
            ...
            wp_enqueue_style(
                ...
                plugins_url('/build/drag-drop-reorder.css', dirname(__FILE__)),
                array(),
                '1.0.0'
            );
        }
    ...
    public function add_settings_to_edit_page() {
        ...
        if ($screen->id === 'edit-post' && isset($_GET['post_status']) && $_GET['post_status'] === 'queued') {
    ...
    public function highlight_queue_menu_item() {
        ...
        if ($pagenow === 'edit.php' && isset($_GET['post_status']) && $_GET['post_status'] === 'queued') {
    ...
    public function conditionally_add_drag_handle_column($columns) {
        if (isset($_GET['post_status']) && $_GET['post_status'] === 'queued') {
    ...
    public function conditionally_populate_drag_handle_column($column, $post_id) {
        if ($column === 'drag_handle' && isset($_GET['post_status']) && $_GET['post_status'] === 'queued') {
