...
// See https://github.com/Automattic/vip-workflow-plugin/blob/trunk/modules/custom-status/lib/hooks/use-intercept-action-dispatch.js
...
export default function useInterceptActionDispatch(
	...
	use( ( registry ) => ( {
		dispatch: ( namespace ) => {
			...
			if (
				! functionOverrides[ storeName ][ actionName ].actionIntercept
			) {
				...
				functionOverrides[ storeName ][ actionName ].actionIntercept = (
					...
					functionOverrides[ storeName ][ actionName ].callback(
						originalAction,
						args
					);
				};
